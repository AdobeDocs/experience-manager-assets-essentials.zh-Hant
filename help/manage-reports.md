...
分配給 [Assets Essentials 管理員產品配置檔案](deploy-administer.md)可以訪問「即時統計」控制面板並在 Assets Essentials 建立用戶定義的報告。
...
瀏覽到 **[!UICONTROL 設定]** > **[!UICONTROL 即時統計]** 查看自動生成的下載資料。
...
1. 在 [!UICONTROL 配置] 頁籤，為報告指定標題和可選說明。
...
之後[建立報告](#create-report)，您可以查看現有報告的清單並選擇以 CSV 格式下載或刪除它們。
...
所選報告以 CSV 格式下載。 CSV 報告中顯示的列取決於您在選擇[建立報告](#create-report)時選擇的資料行。
