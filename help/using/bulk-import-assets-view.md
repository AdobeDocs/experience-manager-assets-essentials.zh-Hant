...
| AWS | <ul> <li>AWS 區域 </li> <li> AWS 貯體 <li> AWS 存取金鑰 </li><li> AWS 存取密碼 </li></ul> |
...
1. 在「**[!UICONTROL 來源資料夾]**」欄位中，提供包含資料來源中資產的根資料夾名稱。
   ...
   >* 如果您在建立Dropbox應用程式時選取&#x200B;**應用程式資料夾**，而且包含資產的資料夾存在於`https://www.dropbox.com/home/Apps/BulkImportAppFolderScope/bulkimport-assets`，請在&#x200B;**[!UICONTROL Source資料夾]**&#x200B;欄位中指定`bulkimport-assets`，其中`BulkImportAppFolderScope`是指應用程式的名稱。`Apps` 在此情況下，會在`home`之後自動新增。
...
當您大量匯入資產或資料夾時，[!DNL Experience Manager Assets]會匯入匯入來源中存在的完整結構。[!DNL Experience Manager] 會遵循資產和檔案夾名稱中特殊字元的內建規則，因此這些檔案名稱需要清除。 對於資料夾名稱和資產名稱，使用者定義的標題保持不變，並且儲存在 `jcr:title` 中。
...
若要查看現有的大量匯入，請選取左窗格中的「**[!UICONTROL 大量匯入]**」選項。 大量匯入頁面會出現，其中包含&#x200B;**[!UICONTROL 已執行的匯入]**&#x200B;清單。<br>
您也可以從下拉式清單選項檢視&#x200B;**[!UICONTROL 儲存的匯入]**&#x200B;和&#x200B;**[!UICONTROL 排程的匯入]**。
