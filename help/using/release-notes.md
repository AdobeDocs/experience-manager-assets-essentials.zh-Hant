...
Experience Manager Assets 現在針對支援的影像格式提供 Content Credentials 支援。 這提供了有關資產沿襲和其創作方式的資訊，包括該資產是否使用生成式 AI 進行了修改。
...
![Assets Firefly 整合](/help/using/assets/assets-firefly-integration.png)
...
「我的工作區」包括用於 Insights、任務和內容的小工具。 您可以根據自己的偏好設定這些小工具在 Workspace 中的顯示方式。
...
Assets Essentials 現在會在您選取使用 Assets Essentials 使用者介面共用資產時，立即自動產生一個連結。 即使您變更過期日，產生的連結仍然有效。
...
資產報告使管理員能夠評估 Adobe Experience Manager Assets Essentials 內的使用者活動。 報告和即時統計儀表板提供有關用戶如何與部署中可用資產交互的有用資訊。[使用報表](manage-reports.md)中的資訊獲取關鍵成功度量，以衡量企業內和客戶對資產的採用程度。
...
* Assets Essentials 目前讓您可以[設定資產的過期日期](manage-organize.md#set-asset-status)。 此外，您可以[](search.md#refine-search-results)根據`Expired`資產狀態和過期日期範圍篩選資產。
...
* 目前已推出 [Assets Essentials 概觀文件](introduction.md)的全新版本。
...
* [!DNL Assets Essentials] 現在支援[建立收藏集](manage-collections.md)。 收藏集是 Experience Manager Assets Essentials 中的一組資產。 使用集合在使用者之間共用資產。 和檔案夾不同，集合可包含來自不同位置的資產。
...
在此版本中，我們將 UI 傳遞的效能和 Assets Essentials 中的特定功能 (對資產和影像編輯進行評論) 最佳化，會依據瀏覽器本機儲存空間和啟用的第三方 cookie 而定。 預設情況下，Chrome 網頁瀏覽器中的無痕模式會封鎖第三方 cookie，使用者若要繼續存取所有功能，有多種選項。
...
* [!DNL Assets Essentials] 現在可讓您[產生連結，並和沒有 [!DNL Assets Essentials] 應用程式的外部利益關係人共用資產](share-links-for-assets.md)。 您可以定義連結的過期日，然後使用您喜歡的通訊方式 (如電子郵件或簡訊服務) 與他人共用。 連結的收件者可預覽和下載資產。
...
* 各種錯誤修復和產品增強功能，包括以前的已知問題（上載<!-- CQ-4337638 -->後，資料夾現在在左側導航樹中正確顯示，拖放上載允許用戶在刪除以供上載時選擇當前資料夾或任何子資料夾）。
