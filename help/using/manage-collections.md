...
將搜尋結果另存為 Smart Collection 以動態更新集合內容。 如果新增到 Assets Essentials 存放庫的資產符合建立智慧型集合時定義的搜尋標準，則開啟智慧型集合時，其內容會自動更新。
...
如需更多有關如何與外部利益關係人共用集合連結的資訊，請參閱[共用資產連結](share-links-for-assets.md)。
...
-->
如需如何為使用者群組指派可用權限的相關資訊，請參閱「[對使用者群組新增權限](/help/using/manage-permissions.md#add-permissions)」。

如要進一步了解端對端工作流程，請參閱「[管理權限](/help/using/manage-permissions.md)」。
